...
#### CENTROIDS
###-------------------
centroids <- apply(data, dim(data)[2], function (x) tapply (x, cluster.dl.cut, mean))
centroids
...
#### Bonferroni for the means within the clusters ###
###-------------------------------------
k <- 3
alpha <- .05
Bfc <- NULL
for (i in 1:3)
{
  ...
  Bf <- cbind(inf = mean - cfr.t*sqrt(diag(cov)/n),
              center = mean, 
              sup = mean + cfr.t*sqrt(diag(cov)/n))
  Bfc <- rbind(Bfc,Bf)
}
