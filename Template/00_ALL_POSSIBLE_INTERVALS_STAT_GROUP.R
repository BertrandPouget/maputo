...
#Bonferroni intervals for the variance
IC.BF.DBOD.Var <- cbind(inf=D.cov[1,1]*(n-1) / qchisq(1 - alpha/(2*k), n-1),
                center=D.cov[1,1],
                sup=D.cov[1,1]*(n-1) / qchisq(alpha/(2*k), n-1))

IC.BF.DSS.Var <- cbind(inf=D.cov[2,2]*(n-1) / qchisq(1 - alpha/(2*k), n-1),
                center=D.cov[2,2],
                sup=D.cov[2,2]*(n-1) / qchisq(alpha/(2*k), n-1))

BfVar <- rbind(IC.BF.DBOD.Var, IC.BF.DSS.Var)
dimnames(BfVar)[[2]] <- c('inf','center','sup')
BfVar
