...
###### ASSUMPTIONS FOR 2-WAYS ANOVA WITH INTERACTIONS (ExAd per fighi!)
lt <- factor(location):factor(type)

# verify assumptions (on the last model):
# 1) normality
Ps={}
for(i in 1:6)
  Ps <- c(Ps,shapiro.test(price[ lt==levels(lt)[i] ])$p)
Ps

# 2) homogeneity of variances
bartlett.test(price, lt)

# Model with interaction (complete model): 
# X.ijk = mu + tau.i + beta.j + gamma.ij + eps.ijk; eps.ijk~N(0,sigma^2), 
#     i=1,2 (effect color), j=1,2,3 (effect region)
fit1 <- aov(price ~ type + location + lt, d) 
summary(fit1)
