...
mcshapiro.test(d2[i1,])$p
mcshapiro.test(d2[i2,])$p

#homogeneity of variances
par(mfrow=c(1,2))
image(cov(d[type=="tourist",1:2]))
image(cov(d[type=="resident",1:2]))
...
# Plot partition in 3 classes
x11()
plot(d, main='Plot', pch=20)
points(d[i1,], col='red', pch=20)
points(d[i2,], col='green', pch=20)
points(d[i3,], col='gold', pch=20)
legend("topright", legend=levels(species.name), fill=c('red','green','gold'), cex=.7)

points(qda.m$means, pch=4,col='black' , lwd=2, cex=1.5)
...
#KNN
knn.m <- knn(train = d, test = xy, cl = species.name, k = k)
z  <- as.numeric(knn.m)
...
x11()
plot(d2, main='Plot', xlab='x1', ylab='x2', pch=20, col=species.name)
legend("topright", legend=levels(species.name), fill=unique(species.name))
...
d.knn <- knn(train = d2, test = xy, cl = species.name, k = k)
z  <- as.numeric(d.knn)
contour(x, y, matrix(z, 200), levels=c(1.5, 2.5), drawlabels=F, add=T)

# error on the training data
data.knn.train <- knn(train = data[,1:2], test = data[,1:2], k = 5, cl = species.name)
misclassif.error <- length(which(species.name != data.knn.train)) / n
misclassif.error
...
for (k in 10:30) {
  knn1 <- knn.cv(train = d[,1:2], cl = species.name, k = k)
  
  errorCV <- (knn1 != species.name)
  err[k]   <- sum(errorCV)/length(d[,1])
}

min(err)
kbest <- which.min(err)  
kbest
best <- knn.cv(train=d[,1:2], cl=d[,3], k=kbest)
errorCV <- (best!=d[,3])
err_fin <- sum(errorCV)/length(d[,3])
err_fin
...
#APER a 3 classi
APER  <- (misc[1,2]*prior.c[1]+misc[1,3]*prior.c[1])/sum(misc[1,]) +
 (misc[2,1]*prior.c[2]+misc[2,3]*prior.c[2])/sum(misc[2,]) +
 (misc[3,1]*prior.c[3]+misc[3,2]*prior.c[3])/sum(misc[3,])
